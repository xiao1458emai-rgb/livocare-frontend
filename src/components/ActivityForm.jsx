// src/components/ActivityForm.jsx - النسخة المصححة (إصلاح مشكلة الألوان)
    ...
    return (
        <div className="activity-form-container">
            ...
            <div className="card">
                <div className="card-header">
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        <h2><span style={{ fontSize: '1.5rem' }}>📝</span> {isArabic ? 'إضافة قياس صحي' : 'Add Health Reading'}</h2>
                        {filledFieldsCount > 0 && <span className="header-badge">📋 {filledFieldsCount}/7 {isArabic ? 'حقول مملوءة' : 'fields filled'}</span>}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        ...
                        {lastAutoSave && <div className="auto-save-time">🕐 {lastAutoSave.toLocaleTimeString(isArabic ? 'ar-EG' : 'en-US')}</div>}
                    </div>
                </div>
                ...
                <div className="esp32-card">
                    ...
                    <div className="esp32-actions">
                        {!sensorActive ? (
                            <button onClick={connectSensor} disabled={sensorConnecting} className="btn-sensor btn-connect">
                                {sensorConnecting ? '⏳ جاري الاتصال...' : '🔌 اتصال ESP32'}
                            </button>
                        ) : (
                            <>
                                <button onClick={disconnectSensor} className="btn-sensor btn-disconnect">🔌 قطع الاتصال</button>
                                <button onClick={fillFormWithSensorData} disabled={sensorHeartRate === null && sensorSpO2 === null} className="btn-sensor btn-fill">📋 {isArabic ? 'تعبئة النموذج' : 'Fill Form'}</button>
                                <button onClick={saveSensorReadingAsHealthRecord} disabled={savingSensorData || (sensorHeartRate === null && sensorSpO2 === null)} className="btn-sensor btn-save">
                        ...
                        <>
                            <div className="sensor-readings">
                                <div className="sensor-reading-card">
                                    ...
                                    <div>
                                        <div className="sensor-label">{isArabic ? 'معدل ضربات القلب' : 'Heart Rate'}</div>
                                        <div><span className="sensor-value">{sensorHeartRate !== null ? sensorHeartRate : '---'}</span> <span className="sensor-unit">BPM</span></div>
                                    </div>
                                </div>
                                <div className="sensor-reading-card">
                                    <span className="sensor-icon">💨</span>
                                    <div>
                                        <div className="sensor-label">{isArabic ? 'نسبة الأكسجين' : 'Oxygen Level'}</div>
                                        <div><span className="sensor-value">{sensorSpO2 !== null ? sensorSpO2 : '---'}</span> <span className="sensor-unit">SpO₂%</span></div>
                                    </div>
                                </div>
                            </div>
                            {lastSensorReading?.timestamp && <div className="sensor-timestamp">🕐 {isArabic ? 'آخر تحديث:' : 'Last update:'} {getLastReadingTime()}</div>}
                        </>
                    )}
                </div>
                ...
                <form onSubmit={handleHealthSubmit}>
                    <div className="form-grid">
                        {[
                            ...
                            <div key={field} className="form-field">
                                <label>
                                    <span>{icon}</span> {label} <span className="label-unit">({unit})</span>
                                </label>
                                <input 
                                    type="number" 
                                    ...
                                    style={validationErrors[field] ? { borderColor: '#ef4444' } : {}}
                                />
                                ...
                                <div className="field-hint">
                                    ✅ {isArabic ? 'الطبيعي' : 'Normal'}: {normal}
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="form-actions">
                        <button type="button" onClick={resetHealthForm} disabled={healthLoading} className="btn-clear">🗑️ {isArabic ? 'مسح النموذج' : 'Clear Form'}</button>
                        <button type="submit" disabled={healthLoading} className="btn-submit">{healthLoading ? '⏳ جاري الحفظ...' : `💾 ${isArabic ? 'حفظ الكل' : 'Save All'}`}</button>
                    </div>
                </form>
            </div>
            ...
            <div className="card activity-card">
                ...
                {userWeight && <div className="weight-info">
                    <span>⚖️</span> <span>{isArabic ? `وزنك: ${userWeight} كجم` : `Your weight: ${userWeight} kg`}</span>
                    <span className="weight-hint">{isArabic ? '(يستخدم لحساب السعرات)' : '(used for calorie calculation)'}</span>
                </div>}
                
                <form onSubmit={handleActivitySubmit}>
                    <select name="activity_type" value={activityFormData.activity_type} onChange={handleActivityChange} required className="form-input" style={{ marginBottom: '0.75rem' }}>
                    ...
                    <input type="number" name="duration_minutes" placeholder={isArabic ? "المدة (دقائق)" : "Duration (minutes)"} value={activityFormData.duration_minutes} onChange={handleActivityChange} required className="form-input" style={{ marginBottom: '0.75rem' }} />
                    ...
                    <div className="calories-input-wrapper">
                        <input 
                            ...
                            className={`calories-input ${showCaloriesEdit ? 'manual' : ''}`}
                        />
                        <span className="calories-unit">{isArabic ? 'سعرة' : 'cal'}</span>
                    </div>
                    ...
                    {!showCaloriesEdit && activityFormData.activity_type && activityFormData.duration_minutes && (
                        <div className="auto-calc-info">
                            ...
                            <button type="button" onClick={() => setShowCaloriesEdit(true)} className="recalc-btn">{isArabic ? 'تعديل يدوي' : 'Edit manually'}</button>
                        </div>
                    )}
                    
                    {showCaloriesEdit && (
                        <div className="manual-edit-info">
                            <span>✏️</span> <span>{isArabic ? 'تم التعديل اليدوي - حساب تلقائي متوقف' : 'Manual edit - auto calculation paused'}</span>
                            <button type="button" onClick={() => { updateCalories(activityFormData.activity_type, activityFormData.duration_minutes); setShowCaloriesEdit(false); }} className="recalc-btn">{isArabic ? 'إعادة الحساب التلقائي' : 'Recalculate'}</button>
                        </div>
                    )}
                    
                    <input type="datetime-local" name="start_time" value={activityFormData.start_time} onChange={handleActivityChange} required className="form-input" style={{ marginBottom: '0.75rem' }} />
                    
                    <button type="submit" disabled={loading} className="btn-submit" style={{ width: '100%' }}>{loading ? 'جاري الحفظ...' : '💾 حفظ النشاط'}</button>
                </form>
            </div>
            ...
            {activities.length > 0 && (
                <div className="card">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                        ...
                        <button onClick={() => setShowCharts(!showCharts)} className="analytics-btn">{showCharts ? '📋 إخفاء' : '📊 إظهار'}</button>
                    </div>
                    
                    {showCharts && (
                        <>
                            <div className="analytics-card">
                                <h4 style={{ margin: '0 0 1rem 0' }}>📊 {isArabic ? 'تحليل الأنشطة' : 'Activity Analytics'}</h4>
                                {activityAnalyticsLoading ? <p>{isArabic ? 'جاري التحليل...' : 'Loading...'}</p> : activityAnalytics ? (
                                    <div className="analytics-grid">
                                        <div><div className="analytics-value">{activityAnalytics.total_activities || 0}</div><div className="analytics-label">{isArabic ? 'عدد الأنشطة' : 'Activities'}</div></div>
                                        <div><div className="analytics-value">{activityAnalytics.total_calories || 0}</div><div className="analytics-label">{isArabic ? 'سعرات حرارية' : 'Calories'}</div></div>
                                        <div><div className="analytics-value">{activityAnalytics.total_duration || 0}</div><div className="analytics-label">{isArabic ? 'دقائق' : 'Minutes'}</div></div>
                                        <div><div className="analytics-value">{activityAnalytics.avg_duration || 0}</div><div className="analytics-label">{isArabic ? 'متوسط المدة' : 'Avg Duration'}</div></div>
                                        <div><div className="analytics-value">{activityAnalytics.avg_calories_per_activity || 0}</div><div className="analytics-label">{isArabic ? 'سعرات/نشاط' : 'Cal/Activity'}</div></div>
                                    </div>
                                ...
                                {Object.entries(chartData.typeStats).map(([type, stats]) => (
                                    <div key={type} className="distribution-item">
                                        <div className="distribution-header">
                                            <span className="distribution-name">{getActivityIcon(type)} {getActivityName(type)} <span className="met-badge">{getMetIcon(type)} {MET_VALUES[type]} MET</span></span>
                                            <span>⏱️ {stats.duration} {isArabic ? 'دقيقة' : 'min'} 🔥 {stats.calories} {isArabic ? 'سعرة' : 'cal'} ({stats.count} {isArabic ? 'نشاط' : 'act'})</span>
                                        </div>
                                        <div className="duration-bar">
                                            <div className="duration-fill" style={{ width: `${maxDuration > 0 ? (stats.duration / maxDuration) * 100 : 0}%` }}>{stats.duration} {isArabic ? 'دقيقة' : 'min'}</div>
                                        </div>
                                        <div className="calories-bar">
                                            <div className="calories-fill" style={{ width: `${maxCalories > 0 ? (stats.calories / maxCalories) * 100 : 0}%` }}>🔥 {stats.calories} {isArabic ? 'سعرة' : 'cal'}</div>
                                        </div>
                                    </div>
                                ))}
                            </div>
                                        ...
                                        return (
                                            <div key={idx} className="chart-bar-wrapper">
                                                <div className="chart-bar" style={{ height: `${height}px` }}>
                                                    <div className="chart-value">🔥{day.calories}</div>
                                                </div>
                                                <div className="chart-label">{day.name}<br /><strong>{day.duration}{isArabic ? 'د' : 'm'}</strong><br /><span className="chart-count">({day.count})</span></div>
                                            </div>
                                        );
                                    })}
                                </div>
                    ...
                    <div className="timeline">
                        ...
                        {activities.map((act, idx) => (
                            <div key={act.id} className="timeline-item">
                                ...
                                <div className={`timeline-card ${idx === 0 ? 'latest' : ''}`}>
                                    <div className="timeline-content">
                                        <div className="timeline-icon">{getActivityIcon(act.activity_type)}</div>
                                        <div className="timeline-info">
                                            <div className="timeline-title">{getActivityName(act.activity_type)}</div>
                                            <div className="timeline-detail">{act.duration_minutes} {isArabic ? 'دقيقة' : 'minutes'}</div>
                                        </div>
                                        {act.calories_burned > 0 && <div className="timeline-calories">🔥 {act.calories_burned} {isArabic ? 'سعرة' : 'cal'}</div>}
                                        <div className="timeline-date">📅 {formatDateTime(act.start_time || act.created_at)}</div>
                                    </div>
                                </div>
                            </div>
                        ))}
            ...
            {message && (
                <div className={`notification-toast ${messageType}`}>
                    ...
                    <span>{message}</span>
                    <button onClick={() => { setMessage(''); setMessageType(''); }} className="toast-close">✕</button>
                </div>
            )}
            ...
            <style jsx>{`
                /* ===========================================
                   ...
                   ✅ جميع الألوان تستخدم المتغيرات CSS
                ...
                [dir="rtl"] .toggle-switch input:checked + .toggle-track .toggle-thumb {
                    transform: translateX(-22px);
                }
                ...
                .dark-mode .calories-recommendation {
                    color: #f1f5f9;
                }
